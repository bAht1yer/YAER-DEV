...
export default async function BlogPost({
  ...
  return (
    <main className="afterimage-site article-content">
      <Navbar />

      <article
        id="main-content"
        tabIndex={-1}
        className="pt-20 pb-20 relative z-10 max-w-4xl mx-auto px-6"
      >
        <Link
          href="/blog"
          className="flex items-center gap-2 text-primary/50 hover:text-primary mb-8 font-mono text-xs uppercase tracking-widest transition-colors w-fit group"
        >
        ...
        <header className="mb-16 border-b border-primary/20 pb-8 relative">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-medium text-white tracking-tighter mb-6 relative">
        ...
        <div className="relative">
          ...
          <div
            className="w-full max-w-full prose prose-invert prose-p:text-gray-300 prose-headings:text-white prose-headings:tracking-tight prose-headings:scroll-mt-32 prose-a:text-primary prose-a:no-underline hover:prose-a:text-white prose-a:transition-colors prose-code:text-primary prose-code:px-1 py-1 prose-code:rounded-sm prose-pre:p-0 prose-pre:bg-transparent prose-pre:border-none prose-pre:shadow-none prose-pre:my-0 [&_figure]:max-w-full [&_pre]:overflow-x-auto [&_pre]:max-w-full [&_pre]:max-h-[500px]"
            dangerouslySetInnerHTML={{ __html: htmlContent }}
          />

          <aside className="absolute inset-y-0 left-full ml-8 hidden w-64 min-[1488px]:block z-20">
            <details
              open
              className="sticky top-32 border-l border-primary/20 pl-6 py-2 group/toc"
            >
              <summary className="text-xs text-primary tracking-widest uppercase mb-4 font-bold cursor-pointer select-none flex items-center justify-between list-none relative h-6 group-open/toc:w-full w-12 -ml-6 pl-6">
                <span className="group-open/toc:block hidden">
                  On this page
                </span>
                ...
                <span className="group-open/toc:hidden absolute left-0 top-0 flex flex-col items-center text-gray-500 hover:text-primary transition-colors h-[60vh] w-12 pt-8">
                  <span
                    ...
                    className="rotate-180 tracking-[0.2em] font-bold"
                  >
                    On this page
                  </span>
                ...
                <span className="group-open/toc:block hidden transition-transform text-gray-500 hover:text-primary">
                  ▼
                </span>
              </summary>
        ...
        {relatedPosts.length > 0 && (
          <div className="mt-24">
            <h3 className="text-xl font-bold text-white uppercase tracking-wider mb-8 flex items-center gap-4">
              <span className="text-primary">&gt;</span> More from the workbench
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedPosts.map((rp) => (
                <Link
                  key={rp.id}
                  href={`/blog/${rp.slug}`}
                  className="group block relative h-32 transition-all duration-500 hover:-translate-y-1 hover-gradient-border border border-primary/20 bg-black/50 p-4 rounded-lg"
                >
