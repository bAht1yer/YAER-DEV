...
export default function BlogFilters({
  ...
  const [results, setResults] = useState<
    { title: string; slug: string; category: { name: string } }[]
  >([]);
  ...
  const handleCategoryClick = (slug: string) => {
    if (slug === "all") {
      ...
      router.replace(`${pathname}?${createQueryString("category", slug)}`, {
        scroll: false,
      });
    }
  };
        ...
        <button
          ...
          className={`text-xs uppercase tracking-widest px-4 py-2 border transition-all ${
            currentCategory === "all"
              ...
              : "border-white/20 text-gray-400 hover:border-primary/50 hover:text-primary/80"
          }`}
        >
          All notes
        </button>
      ...
      <div className="relative w-full md:w-64 search-container">
        <form onSubmit={handleSearchSubmit} className="relative">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <Search
              className={`h-4 w-4 ${isSearching ? "text-primary animate-pulse" : "text-primary/50"}`}
            />
          </div>
          <input
            type="text"
            value={searchTerm}
            onFocus={() =>
              searchTerm.trim() && results.length > 0 && setIsDropdownOpen(true)
            }
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Search notes..."
            className="w-full bg-black border border-white/20 text-primary text-xs tracking-widest uppercase pl-10 pr-4 py-2 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all placeholder:text-gray-400"
          />
        </form>

        {/* Dropdown Results */}
        {isDropdownOpen && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-black border border-primary/30 z-50 shadow-xl rounded overflow-hidden">
            {results.length > 0 ? (
              <div className="py-2">
                <div className="px-4 py-1 text-[10px] text-gray-600 border-b border-white/5 mb-1 tracking-tighter uppercase font-bold">
                  Matching notes
                </div>
                {results.map((post) => (
                  <button
                  ...
                  >
                    ...
                    <span className="text-[10px] text-primary/50 uppercase tracking-widest">
                      CAT: {post.category?.name || "Notes"}
                    </span>
                  </button>
                ))}
              </div>
            ) : searchTerm.trim() && !isSearching ? (
              <div className="px-4 py-4 text-[10px] text-gray-400 text-center uppercase tracking-widest">
                No matching notes
              </div>
            ) : null}
          </div>
